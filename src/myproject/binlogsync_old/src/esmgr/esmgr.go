...
func (es *EsMgr) HandlerSendToEs(q *protocal.SendEsBody) int {
	...
	es.Logger.Infof("HandlerSendtoEs ok q:%+v", q)
...
func (es *EsMgr) HandlerSendToEso(q *protocal.SendEsBody) int {
	...
	url := fmt.Sprintf("http://%v/%s/%s/", es.EsServer, q.Domain, q.Action)
	...
	if res.StatusCode == 201 && res.Status == "201 Created" {
		es.Logger.Infof("HandlerSendtoEs return ok code:%+v, status:%+v",
			res.StatusCode, res.Status)
		return 0
	}

	es.Logger.Infof("HandlerSendtoEs return failed code:%+v, status:%+v",
		res.StatusCode, res.Status)
	return -1
}
