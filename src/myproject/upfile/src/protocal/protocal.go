...
// 通知用户回调接口
type RetCbUploadFile struct {
	Errno  int    `json:"code"`
	Errmsg string `json:"message"`
}

// 创建上传机任务返回结构
type RetTaskAdd struct {
	Errno  string `json:"code"`
	Errmsg string `json:"message"`
}

// 向fastdfs存储数据返回接口
type RetUploadFile struct {
	Errno  int    `json:"code"`
	Errmsg string `json:"message"`
}

// 向fastdfs存储数据请求接口
type CentreUploadFileName struct {
	Taskid   string `json:"taskid"`
