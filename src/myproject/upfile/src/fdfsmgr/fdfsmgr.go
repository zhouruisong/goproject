...
func NewFdfsMgr(trackerlist []string, lg *log.Logger, minConns int, maxConns int) *FdfsMgr {
...
func (fdfs *FdfsMgr) HandlerUploadFile(buf []byte) (int, string) {
	...
	if err != nil {
		fdfs.Logger.Errorf("failed err:%v", err)
		return -1, ""
	}
...
// 处理函数
func (fdfs *FdfsMgr) HandlerUploadByFilename(filename string) (int, string) {
	if filename == "" {
		fdfs.Logger.Errorf("filename len = 0")
		return -1, ""
	}

	uploadres, err := fdfs.pFdfs.UploadByFilename(filename)
	if err != nil {
